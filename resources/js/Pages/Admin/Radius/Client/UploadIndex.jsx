...
function Index({ data, count, page, termSearch, can }) {
    ...
    const table = data.data.map((item, index) => {
        return (
            <tr key={index} className={"border-t transition hover:bg-neutral-100 " + (index % 2 == 0? 'bg-neutral-50': '')}>
                ...
                <td className="px-1 py-3 font-light">{item.client_bond.description}</td>
                <td className="px-1 py-3 font-light">{item.created_at}</td>
                <td className="px-1 py-3 font-light">{item.observations?? '-'}</td>
                <td className="px-1 py-3 font-light">{item.inserted_records}</td>
                <td className="px-1 py-3 font-light">{item.total_records}</td>
            </tr>
        );
    });
    ...
    return (
        <>
            <AuthenticatedLayout
                titleChildren={'Gerenciamento de Uploads de Clientes'}
                breadcrumbs={[
                    { label: 'Clientes', url: route('clients.index') },
                    { label: 'Uploads', url: route('clients.uploadIndex') },
                ]}
            >
                <div className="flex gap-2 md:flex-row md:gap-4">
                    {can.create && <Panel className={'inline-flex'}>
                        <Link href={route('clients.uploadCreate')} className="inline-flex items-center justify-between gap-2 px-3 py-2 font-light text-white transition bg-blue-500 border border-transparent rounded-md focus:ring hover:bg-blue-600 focus:ring-sky-300">
                    ...
                    <table className="w-full table-auto text-neutral-600">
                        <thead>
                            <tr className="border-b">
                                ...
                                <th className="px-1 pt-3 font-semibold text-left">Grupo de Clientes</th>
                                <th className="px-1 pt-3 font-semibold text-left">Data</th>
                                <th className="px-1 pt-3 font-semibold text-left">Observações</th>
                                <th className="px-1 pt-3 font-semibold text-left">Inserções</th>
                                <th className="px-1 pt-3 font-semibold text-left">Linhas no arquivo</th>
                            </tr>
                        </thead>
                        <tbody>
                            {table}
